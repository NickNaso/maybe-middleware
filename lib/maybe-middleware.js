 ...
 */
module.exports = function maybeMiddleWare (middleware, predicate) {
  if (_.isFunction(middleware)) {
    return dispatchError(new MaybeMiddlewareError("The middleware parameter " + 
           "must be a function. See: http://expressjs.com ")); 
  }
  if (predicate && _.isBoolean(predicate)) {
    return predicate ? maybeMiddleWare : noMiddleware;
  } else if (predicate && _.isFunction(predicate)) {
      return predicate.call() ? maybeMiddleWare : noMiddleware;
  } else {
